import type { TileRange, TilesDeletionParams } from '@map-colonies/raster-shared';
...
function tilePathsForRange(range: TileRange, tilesPath: string, fileExtension: string): string[] {
  const paths: string[] = [];
  for (let x = range.minX; x <= range.maxX; x++) {
    for (let y = range.minY; y <= range.maxY; y++) {
      paths.push(`${tilesPath}/${range.zoom}/${x}/${y}.${fileExtension}`);
    }
  }
  return paths;
}

function tilePathsForRanges(params: TilesDeletionParams): string[] {
  return params.ranges.flatMap((range) => tilePathsForRange(range, params.tilesPath, params.fileExtension));
}
 ...
 */
function extraTilePathsAroundRanges(params: TilesDeletionParams, padding: number): string[] {
  const targetSet = new Set(tilePathsForRanges(params));
  const extras: string[] = [];
  for (const range of params.ranges) {
    const wider = paddedRange(range, padding);
    for (const path of tilePathsForRange(wider, params.tilesPath, params.fileExtension)) {
 ...
 * */
function extraTilePathsAtAdjacentZooms(params: TilesDeletionParams, zoomDeltas: number[]): string[] {
  const extras: string[] = [];
  for (const range of params.ranges) {
    for (const delta of zoomDeltas) {
      const zoom = range.zoom + delta;
      if (zoom < 0) continue;
      extras.push(`${params.tilesPath}/${zoom}/${range.minX}/${range.minY}.${params.fileExtension}`);
    }
  }
  return extras;
}
